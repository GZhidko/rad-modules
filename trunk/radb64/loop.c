...
int (*processor)(char *, char *) = b64_enc;
...
void reply(char *s, char *slim) {
  ...
  r = repbuff;
  while ((s = get_str(s, slim, str)) < slim) {
    if (debug) write_error("KEY: %s\n", str);
    processor(str, rep);
    if (debug) write_error("VALUE: [%s]\n", rep);
    l = strlen(rep);
    if (r - repbuff + l + 5 + 2 + 7 > RPBUFFSIZE) {
      write_error("OUTLET OVERFILL\n");
      exit(1);  /* DIE */
    }
    strcpy(r, "str=\""), r += 5;
    strcpy(r, rep), r += l;
  ...
  strcpy(r, "int=1\n\n"), r += 7;
  len = r - repbuff;
  wlen = write(1, repbuff, len);
  if (len != wlen) {
    write_error("Server does not accept all data (data=%db; post=%db)\n", len, wlen);
...
void main_loop() {
  ...
  size_t n, l;
  stats_c_t stats;
  stats_c_init(&stats, "radb64", stats_file, stats_interval_min);
  buff = rq;
  nl_cnt = 0;
  rq_start = rq;
  for (;;) {
